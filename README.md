...
- **多风格**：Windows 2000 / Windows XP / Windows 3.1 三种系统风格，每种都有浅色和深色两套配色；管理员可以在后台设置默认风格，访客可以在标题栏自行切换。
...
- **站点默认风格**：在后台 → 外观 → 「主题自定义配置 JSON」中填写：
  ...
  可选值为 `win2000`（默认）、`xp`、`win31`，填写其他值时按 `win2000` 处理。如果 JSON 里已经有别的配置，把这一项加进同一个对象即可。
