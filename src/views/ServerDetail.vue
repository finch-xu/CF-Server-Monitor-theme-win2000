<template>
  <div class="page">
    <nav class="toolbar">
      <router-link to="/" class="tool-btn"><WinIcon name="back" :size="16" />{{ trans.back }}</router-link>
      <span class="tool-sep" aria-hidden="true"></span>
      <span class="tool-label">{{ trans.w2kHistoryRange }}</span>
      <button
        v-for="option in timeOptions"
        :key="option.hours"
        type="button"
        class="tool-btn"
        :class="{ active: currentHours === option.hours, disabled: option.hours > 24 && !loggedIn }"
        :aria-pressed="currentHours === option.hours ? 'true' : 'false'"
    ...
    <div class="page-content">
      ...
      <template v-else>
        <fieldset class="groupbox host-box">
          <legend><b>{{ trans.w2kHostInfo }}</b></legend>
          <div class="host-head">
            <RegionFlag :region="server.region" />
            <OsIcon :os="server.os" />
            <h2 class="host-name">{{ server.name || serverId }}</h2>
            <span class="thin-sunken host-status">
              <span class="led" :class="{ on: isOnline }"></span>{{ isOnline ? trans.online : trans.offline }}
            </span>
          </div>
          <dl class="sunken host-grid">
            <div class="host-item">
              <dt>{{ trans.uptime }}</dt>
              <dd>{{ formatUptime(server.boot_time) }}</dd>
            </div>
            <div v-if="server.expire_date" class="host-item">
              <dt>{{ trans.expire }}</dt>
              <dd :class="{ 'text-offline': isExpired }">{{ expireDaysText }}</dd>
            </div>
            <div class="host-item">
              <dt>{{ trans.os }} / {{ trans.architecture }}</dt>
              <dd>{{ server.os || 'N/A' }} / {{ server.arch || 'N/A' }}</dd>
            </div>
            <div v-if="server.kernel_version" class="host-item">
              <dt>{{ trans.kernelVersion || 'Kernel' }}</dt>
              <dd>{{ server.kernel_version }}</dd>
            </div>
            <div class="host-item">
              <dt>{{ trans.cpuInfo }}</dt>
              <dd>{{ server.cpu_info || 'N/A' }} x {{ server.cpu_cores || 'N/A' }}</dd>
            </div>
            <div v-if="hasGpuData" class="host-item">
              <dt>{{ trans.gpuInfo || 'GPU Info' }}</dt>
              <dd>{{ gpuInfoText }}</dd>
            </div>
            <div class="host-item">
              <dt>{{ trans.totalDiskRam }}</dt>
              <dd>{{ formatBytes(server.disk_total * 1024 * 1024) }} / {{ formatBytes(server.ram_total * 1024 * 1024) }}</dd>
            </div>
            <div class="host-item">
              <dt>{{ trans.loadAvg }}</dt>
              <dd class="num">{{ server.load_avg || '0.00 0.00 0.00' }}</dd>
            </div>
            <div class="host-item">
              <dt>{{ trans.totalTraffic }}</dt>
              <dd class="num">▼ {{ formatBytes(server.net_rx) }} / ▲ {{ formatBytes(server.net_tx) }}</dd>
            </div>
            <div class="host-item">
              <dt>{{ trans.realtimeSpeed }}</dt>
              <dd class="num">▼ {{ formatBytes(server.net_in_speed) }}/s / ▲ {{ formatBytes(server.net_out_speed) }}/s</dd>
            </div>
            <div v-if="server.net_rx_monthly" class="host-item">
              <dt>{{ trans.monthlyTraffic }}</dt>
              <dd class="num">▼ {{ formatBytes(server.net_rx_monthly) }} / ▲ {{ formatBytes(server.net_tx_monthly) }}</dd>
            </div>
            <div v-if="server.net_rx_monthly" class="host-item">
              <dt>{{ trans.monthlyTrafficLimit }}</dt>
              <dd class="num">{{ formatBytes(trafficUsageBytes) }} / {{ server.traffic_limit ? formatBytes(server.traffic_limit * 1024 * 1024 * 1024) : trans.w2kUnlimited }}</dd>
            </div>
            <div class="host-item">
              <dt>{{ trans.bootTime }}</dt>
              <dd>{{ formatTimestamp(server.boot_time) }}</dd>
            </div>
            <div class="host-item">
              <dt>{{ trans.lastUpdate }}</dt>
        ...
        <div class="charts-grid">
          <fieldset class="groupbox chart-box" :class="{ 'full-width': isChartExpanded('cpu') }">
            <legend>{{ trans.cpuUsage }}</legend>
            <div class="chart-head">
              <span class="chart-values"><b>{{ cpuPercent }}%</b></span>
              <ChartExpandButton :expanded="isChartExpanded('cpu')" @toggle="toggleChartExpanded('cpu')" />
            </div>
            <div class="thin-sunken graph"><canvas ref="cpuChartRef"></canvas></div>
          </fieldset>

          <fieldset class="groupbox chart-box" :class="{ 'full-width': isChartExpanded('ram') }">
            <legend>{{ trans.memoryUsage }}</legend>
            <div class="chart-head">
              <span class="chart-values"><b>{{ ramPercent }}%</b> · {{ trans.swap }} {{ server.swap_used || '0' }} / {{ server.swap_total || '0' }} MiB</span>
              <ChartExpandButton :expanded="isChartExpanded('ram')" @toggle="toggleChartExpanded('ram')" />
            </div>
            <div class="thin-sunken graph"><canvas ref="ramChartRef"></canvas></div>
          </fieldset>

          <fieldset class="groupbox chart-box" :class="{ 'full-width': isChartExpanded('net') }">
            <legend>{{ trans.networkTraffic }}</legend>
            <div class="chart-head">
              <span class="chart-values"><span class="net-down">▼ {{ formatBytes(server.net_in_speed) }}/s</span> <span class="net-up">▲ {{ formatBytes(server.net_out_speed) }}/s</span></span>
              <ChartExpandButton :expanded="isChartExpanded('net')" @toggle="toggleChartExpanded('net')" />
            </div>
            <div class="thin-sunken graph"><canvas ref="netChartRef"></canvas></div>
          </fieldset>

          <fieldset class="groupbox chart-box" :class="{ 'full-width': isChartExpanded('load') }">
            <legend>{{ trans.loadAvgMonitor }}</legend>
            <div class="chart-head">
              <span class="chart-values">
                {{ trans.load1m }} <b>{{ (parseLoadAvg(server.load_avg)[0] || 0).toFixed(2) }}</b>
                {{ trans.load5m }} <b>{{ (parseLoadAvg(server.load_avg)[1] || 0).toFixed(2) }}</b>
                {{ trans.load15m }} <b>{{ (parseLoadAvg(server.load_avg)[2] || 0).toFixed(2) }}</b>
              </span>
              <ChartExpandButton :expanded="isChartExpanded('load')" @toggle="toggleChartExpanded('load')" />
            </div>
            <div class="thin-sunken graph"><canvas ref="loadChartRef"></canvas></div>
          </fieldset>

          <fieldset class="groupbox chart-box" :class="{ 'full-width': isChartExpanded('disk') }">
            <legend>{{ trans.diskUsage }}</legend>
            <div class="chart-head">
              <span class="chart-values"><b>{{ diskPercent }}%</b> · {{ trans.used }} {{ formatBytes(server.disk_used * 1024 * 1024) }} / {{ formatBytes(server.disk_total * 1024 * 1024) }}</span>
              <ChartExpandButton :expanded="isChartExpanded('disk')" @toggle="toggleChartExpanded('disk')" />
            </div>
            <div class="thin-sunken graph"><canvas ref="diskChartRef"></canvas></div>
          </fieldset>

          <fieldset v-show="hasDiskIoData" class="groupbox chart-box" :class="{ 'full-width': isChartExpanded('diskIo') }">
            <legend>{{ trans.diskIo || 'Disk IO' }}</legend>
            <div class="chart-head">
              <span class="chart-values">
                R <b>{{ formatBytes(diskIo.read_bps) }}/s</b>
                W <b>{{ formatBytes(diskIo.write_bps) }}/s</b>
                IOPS <b>{{ formatDiskIoNumber(diskIo.read_iops) }}/{{ formatDiskIoNumber(diskIo.write_iops) }}</b>
                await <b>{{ formatDiskIoNumber(diskIo.await_ms) }}ms</b>
                util <b>{{ formatDiskIoNumber(diskIo.util) }}%</b>
              </span>
              <ChartExpandButton :expanded="isChartExpanded('diskIo')" @toggle="toggleChartExpanded('diskIo')" />
            </div>
            <div class="thin-sunken graph"><canvas ref="diskIoChartRef"></canvas></div>
          </fieldset>

          <fieldset v-show="hasGpuData" class="groupbox chart-box" :class="{ 'full-width': isChartExpanded('gpu') }">
            <legend>{{ trans.gpuUsage || 'GPU Usage' }}</legend>
            <div class="chart-head">
              <span class="chart-values"><b>{{ gpuPercentText }}</b></span>
              <ChartExpandButton :expanded="isChartExpanded('gpu')" @toggle="toggleChartExpanded('gpu')" />
            </div>
            <div class="thin-sunken graph"><canvas ref="gpuChartRef"></canvas></div>
          </fieldset>

          <fieldset v-show="hasGpuMemData" class="groupbox chart-box" :class="{ 'full-width': isChartExpanded('gpuMem') }">
            <legend>{{ trans.gpuMemory || 'GPU Memory' }}</legend>
            <div class="chart-head">
              <span class="chart-values"><b>{{ gpuMemText }}</b></span>
              <ChartExpandButton :expanded="isChartExpanded('gpuMem')" @toggle="toggleChartExpanded('gpuMem')" />
            </div>
            <div class="thin-sunken graph"><canvas ref="gpuMemChartRef"></canvas></div>
          </fieldset>

          <fieldset v-show="hasGpuClockData" class="groupbox chart-box" :class="{ 'full-width': isChartExpanded('gpuClock') }">
            <legend>{{ trans.gpuClock || 'GPU SM Clock' }}</legend>
            <div class="chart-head">
              <span class="chart-values"><b>{{ gpuClockText }}</b></span>
              <ChartExpandButton :expanded="isChartExpanded('gpuClock')" @toggle="toggleChartExpanded('gpuClock')" />
            </div>
            <div class="thin-sunken graph"><canvas ref="gpuClockChartRef"></canvas></div>
          </fieldset>

          <fieldset v-show="hasGpuPowerData" class="groupbox chart-box" :class="{ 'full-width': isChartExpanded('gpuPower') }">
            <legend>{{ trans.gpuPower || 'GPU Power' }}</legend>
            <div class="chart-head">
              <span class="chart-values"><b>{{ gpuPowerText }}</b></span>
              <ChartExpandButton :expanded="isChartExpanded('gpuPower')" @toggle="toggleChartExpanded('gpuPower')" />
            </div>
            <div class="thin-sunken graph"><canvas ref="gpuPowerChartRef"></canvas></div>
          </fieldset>

          <fieldset class="groupbox chart-box" :class="{ 'full-width': isChartExpanded('proc') }">
            <legend>{{ trans.processes }}</legend>
            <div class="chart-head">
              <span class="chart-values"><b>{{ server.processes || '0' }}</b></span>
              <ChartExpandButton :expanded="isChartExpanded('proc')" @toggle="toggleChartExpanded('proc')" />
            </div>
            <div class="thin-sunken graph"><canvas ref="procChartRef"></canvas></div>
          </fieldset>

          <fieldset class="groupbox chart-box" :class="{ 'full-width': isChartExpanded('conn') }">
            <legend>{{ trans.connections }}</legend>
            <div class="chart-head">
              <span class="chart-values">TCP <b>{{ server.tcp_conn || '0' }}</b> UDP <b>{{ server.udp_conn || '0' }}</b></span>
              <ChartExpandButton :expanded="isChartExpanded('conn')" @toggle="toggleChartExpanded('conn')" />
            </div>
            <div class="thin-sunken graph"><canvas ref="connChartRef"></canvas></div>
          </fieldset>

          <fieldset v-show="hasPingData" class="groupbox chart-box" :class="{ 'full-width': isChartExpanded('ping') }">
            <legend>{{ trans.latencyMonitor }}</legend>
            <div class="chart-head">
              <span class="chart-values">
                <span v-for="item in visiblePingStats" :key="item.field" class="chart-value-item">
            ...
            <div class="thin-sunken graph"><canvas ref="pingChartRef"></canvas></div>
          </fieldset>

          <fieldset v-show="hasLossData" class="groupbox chart-box" :class="{ 'full-width': isChartExpanded('loss') }">
            <legend>{{ trans.packetLoss || 'Packet Loss' }}</legend>
            <div class="chart-head">
              <span class="chart-values">
                <span v-for="item in visibleLossStats" :key="item.field" class="chart-value-item">
                  {{ item.label }} <b>{{ item.value }}%</b>
                </span>
              </span>
              <ChartExpandButton :expanded="isChartExpanded('loss')" @toggle="toggleChartExpanded('loss')" />
            </div>
            <div class="thin-sunken graph"><canvas ref="lossChartRef"></canvas></div>
          </fieldset>
        </div>
      </template>
    </div>
    ...
    >
      {{ trans.loginRequired }}
      <template #buttons>
        <button type="button" class="btn default" @click="goToLogin">{{ trans.login }}</button>
        <button type="button" class="btn" @click="showLoginModal = false">{{ trans.cancel }}</button>
      </template>
    </WinDialog>

    <WinDialog
      :show="showDbUpgradeDialog"
      :title="trans.w2kError"
      icon="warning"
      :close-label="trans.close"
      @close="showDbUpgradeDialog = false"
    >
      {{ t('databaseUpgradeRequired') }}
      <template #buttons>
        <button type="button" class="btn default" @click="showDbUpgradeDialog = false">{{ trans.w2kOk }}</button>
      </template>
    </WinDialog>
...
<script setup>
// 数据、图表与实时逻辑移植自 CF-Server-Monitor 默认主题 ServerDetail.vue（MIT License, https://github.com/huilang-me/CF-Server-Monitor）
...
import { fetchServerDetail, fetchAllHistory, fetchConfig, formatBytes, isAdminLoggedIn, createLiveSocket, isServerOnline, normalizeLiveSocketTimeoutMinutes } from '../utils/api.js'
import { getTrafficUsageBytes } from '../composables/useServerCardData'
import { getAdminUrl, getTitle, hasMultipleApiBases } from '../utils/config.js'
import { Chart, registerables } from 'chart.js'
import { t, currentLang, useTranslation } from '../utils/i18n'
import { CHART, DEFAULT_SITE_TITLE, HISTORY } from '../utils/constants'
...
const showDbUpgradeDialog = ref(false)
...
const ChartExpandButton = {
  ...
  emits: ['toggle'],
  setup(props, { emit }) {
    const getLabel = () => props.expanded ? trans.value.w2kCollapse : trans.value.w2kExpand
    return () => h('button', {
      type: 'button',
      class: 'title-btn chart-expand-btn',
      title: getLabel(),
      'aria-label': getLabel(),
      'aria-pressed': props.expanded ? 'true' : 'false',
      onClick: () => emit('toggle')
    }, [h(WinIcon, { name: props.expanded ? 'restore' : 'expand', size: props.expanded ? 9 : 8 })])
  }
}
...
watch([() => server.value.name, siteTitle], () => {
  const name = server.value.name || ''
  setWindowTitle(name ? `${name} - ${siteTitle.value}` : siteTitle.value)
}, { immediate: true })

watch([loading, liveConnected, lastUpdateText, currentLang], () => {
  if (loading.value) {
...
const timeOptions = computed(() => {
  ...
  return [
    { hours: REALTIME_HISTORY_HOURS, label: zh ? '10 分钟' : '10m' },
    { hours: 0.5, label: zh ? '30 分钟' : '30m' },
    { hours: 1, label: zh ? '1 小时' : '1h' },
    { hours: 6, label: zh ? '6 小时' : '6h' },
    { hours: 12, label: zh ? '12 小时' : '12h' },
    { hours: 24, label: zh ? '24 小时' : '24h' },
    { hours: 48, label: zh ? '2 天' : '2d' },
    { hours: 96, label: zh ? '4 天' : '4d' },
    { hours: 168, label: zh ? '7 天' : '7d' }
  ]
})
  ...
  try {
    ...
    if (e && e.message === 'databaseUpgradeRequired') {
      if (!databaseUpgradeAlertShown) {
        ...
        showDbUpgradeDialog.value = true
      }
      return
    }
    historyLoaded.value = true
...
const goToLogin = () => {
  ...
  window.location.href = getAdminUrl(apiIndex.value)
}
...
const handleVisibility = () => {
  ...
  if (document.hidden) {
    ...
    fetchCurrentStatus().finally(() => liveSocket?.reconnect())
  }
}
